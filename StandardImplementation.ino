/**
 * Solar Power Management System with Temperature Monitoring
 * 
 * This Arduino firmware manages a solar power system with voltage regulation,
 * temperature monitoring, Bluetooth communication, and power-saving features.
 * 
 ...
 * Monitors temperature, manages power saving, and controls voltage regulation
 */
void loop() {
  ...
  // Transmit temperature data if it exceeds danger threshold or has changed significantly
  if((steinhart >= dangerousTemp) || ((fabs(steinhart - prevTemp)) > tempThreshold)) {
    digitalWrite(bluetoothPin, HIGH);  // Enable Bluetooth module
    delay(125);
    
    // Send temperature data to both serial interfaces
    Serial.println(steinhart);
    mySerial.println(steinhart);
